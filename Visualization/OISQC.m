...
while 1
    ...
    if exist([directory, filename,num2str(run),'_DataVis.jpg'],'file')
        ...
        if strcmp(suffix, 'stim')
            ...
            save([directory, filename,num2str(run),'-datahb'], 'AvgOxy', 'AvgDeOxy', 'AvgTotal', 'Oxy', 'DeOxy', '-append');
        ...
        figure('visible', 'on');
        close all
        save([directory, filename,num2str(run),'-datahb'], 'mdatanorm', 'mdata', 'stddatanorm', 'LTMvMt', 'InstMvMt', 'Shift', 'time', 'fdata', 'hz', '-append');
        run=run+1;
    end
end
